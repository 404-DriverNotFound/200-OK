...
void		ServerManager::fdCopy(SetType fdset)
{
	...
	if (fdset == READ_SET || fdset == ALL_SET)
	{
		...
		cout << "&m_read_copy_set" << &m_read_copy_set << endl;
	}
}
...
int ServerManager::SetServers()
{
	for (size_t i = 0; i < this->m_servers.size(); i++)
	{
		...
		cout << "server.m_manager = this; " << server.m_manager << endl;
...
fd_set& ServerManager::GetReadCopySet()
{return (this->m_read_copy_set);}
