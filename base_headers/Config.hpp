...
class Config
{
	...
	public:
		...
		// FIXME 아래 함수를 클래스 내부로 넣고 싶은데 클래스의 상호참조 문제때문에 컴파일이 안됨.
		// int parsingServerBlock(std::vector<std::string> &gnl, int start, int end, ServerBracket *ServerBracket);
		// int parsingLocationBlock(std::vector<std::string> &gnl, int start, int end);
};